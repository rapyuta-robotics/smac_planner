...
struct SmootherParams
{
  ...
  void get(rclcpp_lifecycle::LifecycleNode * node, const std::string & name)
  {
    ...
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "w_curve", ros::ParameterValue(1.5));
    node->get_parameter(local_name + "w_curve", curvature_weight);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "w_cost", ros::ParameterValue(0.0));
    node->get_parameter(local_name + "w_cost", costmap_weight);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "w_dist", ros::ParameterValue(0.0));
    node->get_parameter(local_name + "w_dist", distance_weight);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "w_smooth", ros::ParameterValue(15000.0));
    node->get_parameter(local_name + "w_smooth", smooth_weight);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "cost_scaling_factor", ros::ParameterValue(10.0));
...
struct OptimizerParams
{
  ...
  void get(rclcpp_lifecycle::LifecycleNode * node, const std::string & name)
  {
    ...
    // Optimizer params
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "param_tol", ros::ParameterValue(1e-15));
    node->get_parameter(local_name + "param_tol", param_tol);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "fn_tol", ros::ParameterValue(1e-7));
    node->get_parameter(local_name + "fn_tol", fn_tol);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "gradient_tol", ros::ParameterValue(1e-10));
    node->get_parameter(local_name + "gradient_tol", gradient_tol);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "max_iterations", ros::ParameterValue(500));
    node->get_parameter(local_name + "max_iterations", max_iterations);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "max_time", ros::ParameterValue(0.100));
    node->get_parameter(local_name + "max_time", max_time);
    nav2_util::declare_parameter_if_not_declared(
      node, local_name + "debug_optimizer", ros::ParameterValue(false));
