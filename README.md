...
planner_server:
  ros__parameters:
    ...
    GridBased:
      ...
      allow_goal_overshoot: false         # For Hybrid: If false, don't search in cells which are on opposite side of the goal pose, compared to the side where the start pose is.
      goal_align_distance: 0.0            # For Hybrid: The minimum distance from the goal pose at which the robot pose should be aligned to the goal pose (aligned according to tolerance).
      ...
      lookup_table_size: 20.0               # For Hybrid nodes: Size of the dubin/reeds-shepp distance window to cache, in meters.
